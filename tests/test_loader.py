...
def gestures_to_canonical(mouse, keyboard):
    ...
    for m in mouse:
        ...
        result[m.callback]["mouse"]["conditions"] = [c.model_dump() for c in m.conditions]
...
def test_load_keyboard_mouse_roundtrip(gesture_config):
    ...
    assert sorted(canonical, key=lambda x: x["callback"]) == sorted(expected, key=lambda x: x["callback"])


def test_callbacks_preserved(gesture_config):
    bundle = parse_shortcut_config(gesture_config)

    loaded_callbacks = {g.callback for g in bundle.mouse_gestures} | {
        g.callback for g in bundle.keyboard_gestures
    }

    original_callbacks = {item["callback"] for item in gesture_config}
...
def test_mouse_conditions_structure(gesture_config):
    ...
    source_lookup = {item["callback"]: item["mouse"]["conditions"] for item in gesture_config}
...
def test_alignment_by_callback(gesture_config):
    ...
    config_mouse_callbacks = {item["callback"] for item in gesture_config if item["mouse"]["conditions"]}

    config_keyboard_callbacks = {item["callback"] for item in gesture_config if item["keyboard"]["conditions"]}
